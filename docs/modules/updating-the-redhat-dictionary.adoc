...
The Red Hat dictionary is a Hunspell-format dictionary that contains technical terms, product names, and Red Hat-specific jargon. When a word is missing from the dictionary and is common enough to benefit all Red Hat documentation projects, you can add it to the dictionary wordlist.
...
. Open the wordlist file `.vale/styles/config/dictionaries/wordlist.txt` in a text editor.
...
The affix file `.vale/styles/config/dictionaries/en_US-RedHat.aff` defines the morphology rules that the dictionary flags reference.
...
* link:{repository-url}blob/main/.vale/styles/config/dictionaries/wordlist.txt[`wordlist.txt` source code]
* link:{repository-url}blob/main/.vale/styles/config/dictionaries/en_US-RedHat.dic[`en_US-RedHat.dic` generated dictionary]
