...
[id="asciidoc-openshift-style-for-vale"]
...
. Update your project `.vale.ini` to include the `OpenShiftAsciiDoc` style. Add `OpenShiftAsciiDoc` to the `Packages` and `BasedOnStyles` fields. For example:
+
.Example .vale.ini
[source,ini]
----
...
The xref:contributor-guide.adoc#vale-at-red-hat-repository[Vale at Red Hat repository] contains the `OpenShiftAsciiDoc` style in the following directory:
